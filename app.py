...
def gerar_pdf(registro):
    try:
        ...
        try:
            ...
            c.drawImage(logo, margem_esquerda, y - 40, width=80, height=40, mask='auto')
        except:
            pass

        c.setFont("Helvetica-Bold", 16)
        c.setFillColorRGB(15/255, 42/255, 77/255)  # Azul institucional
        c.drawCentredString(largura / 2, y, "Diário de Obra - RDV Engenharia")
        y -= 50
        c.setFillColorRGB(0, 0, 0)  # Volta para preto padrão
        ...
        try:
            ...
            c.drawString(margem_esquerda + 10, y, "Erro ao carregar efetivo")
            y -= 15

        c.drawString(margem_esquerda, y, f"Ocorrências: {registro['Ocorrências']}")
        y -= 20
        c.drawString(margem_esquerda, y, f"Responsável Empresa: {registro['Responsável Empresa']}")
        y -= 20
