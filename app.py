...
# Leitura das listas de colaboradores, obras e contratos
...
data = st.date_input("Data", value=datetime.today(), format="%d/%m/%Y")
...
def gerar_pdf():
    try:
        ...
        if texto_efetivo == "" or texto_efetivo.lower() == "nan":
            st.warning("Registro ignorado por erro no campo Efetivo: vazio ou inválido")
            return
